...
func TestGetPluginLocation(t *testing.T) {
	...
	location, err := GetPluginLocation("github.com/argoproj-labs/http")
	assert.NoError(t, err)
	fp, err := filepath.Abs(filepath.Join(defaults.DefaultRolloutPluginFolder, "github.com/argoproj-labs/http"))
	assert.NoError(t, err)
