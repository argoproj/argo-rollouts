...
import (
	...
	"github.com/argoproj/argo-rollouts/utils/cache"
...
func TestIDTokenClaims(t *testing.T) {
	oauth2Config := &oauth2.Config{
		...
		Endpoint:     oauth2.Endpoint{AuthURL: "https://argorollouts-dev.onelogin.com/oidc/auth", TokenURL: "https://argorollouts-dev.onelogin.com/oidc/token"},
		Scopes:       []string{"oidc", "profile", "groups"},
		RedirectURL:  "https://argorollouts-dev.io/redirect_url",
	}
...
func (p *fakeProvider) Verify(_ string, _ *settings.ArgoRolloutsSettings) (*gooidc.IDToken, error) {
	return nil, nil
}
...
func TestClientApp_HandleLogin(t *testing.T) {
	...
	t.Run("oidc certificate checking during login should toggle on config", func(t *testing.T) {
		cdSettings := &settings.ArgoRolloutsSettings{
			URL: "https://argorollouts.example.com",
...
requestedScopes: ["oidc"]`, oidcTestServer.URL),
		...
		app, err := NewClientApp(cdSettings, dexTestServer.URL, nil, "https://argorollouts.example.com", cache.NewInMemoryCache(24*time.Hour))
		require.NoError(t, err)

		req := httptest.NewRequest(http.MethodGet, "https://argorollouts.example.com/auth/login", nil)
...
- type: github
  ...
  config:
    ...
    clientSecret: aabbccddeeff00112233`,
		...
		app, err = NewClientApp(cdSettings, dexTestServer.URL, &dex.DexTLSConfig{StrictValidation: false}, "https://argorollouts.example.com", cache.NewInMemoryCache(24*time.Hour))
		require.NoError(t, err)
...
requestedScopes: ["oidc"]`, oidcTestServer.URL),
	...
	req = httptest.NewRequest(http.MethodGet, fmt.Sprintf("https://argorollouts.example.com/auth/callback?state=%s&code=abc", state), nil)
...
requestedScopes: ["oidc"]`, oidcTestServer.URL),
		...
		req := httptest.NewRequest(http.MethodGet, "https://argorollouts.example.com/auth/callback", nil)
...
func TestIsValidRedirect(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			allowedURLs: []string{"https://localhost:4000/argorollouts"},
		},
		{
			name:        "Valid redirect URL because prefix matches",
			valid:       true,
			redirectURL: "https://localhost:4000/argorollouts/applications",
			allowedURLs: []string{"https://localhost:4000/argorollouts"},
		},
		{
		...
		},
		{
			...
			valid:       false,
			redirectURL: "https://localhost:80/argorollouts\r\n",
			allowedURLs: []string{"https://localhost:80/argorollouts\r\n"},
		},
	}
...
func TestGenerateAppState(t *testing.T) {
	...
	require.NoError(t, err)
	expectedReturnURL := "http://argorollouts.example.com/"
	app, err := NewClientApp(&settings.ArgoRolloutsSettings{ServerSignature: signature, URL: expectedReturnURL}, "", nil, "", cache.NewInMemoryCache(24*time.Hour))
	require.NoError(t, err)
...
func TestGenerateAppState_XSS(t *testing.T) {
	...
	t.Run("valid return URL succeeds", func(t *testing.T) {
		expectedReturnURL := "https://argorollouts.example.com/some/path"
		generateResponse := httptest.NewRecorder()
		state, err := app.generateAppState(expectedReturnURL, generateResponse)
		require.NoError(t, err)
...
func TestGenerateAppState_NoReturnURL(t *testing.T) {
	signature, err := util.MakeSignature(32)
	require.NoError(t, err)
	cdSettings := &settings.ArgoRolloutsSettings{ServerSignature: signature}
