...
import (
	...
	"github.com/argoproj/argo-rollouts/utils/assets"
	"github.com/argoproj/argo-rollouts/utils/glob"
...
func (e *Enforcer) getCabinEnforcer(policy string) CasbinEnforcer {
	res, err := e.tryGetCabinEnforcer(policy)
	if err != nil {
		panic(err)
	}
	return res
...
func (e *Enforcer) tryGetCabinEnforcer(policy string) (CasbinEnforcer, error) {
	e.lock.Lock()
	defer e.lock.Unlock()
	...
	if policy != "" {
		if enforcer, err = newEnforcerSafe(matchFunc, e.model, newAdapter(e.adapter.builtinPolicy, e.adapter.userDefinedPolicy, policy)); err != nil {
			...
			log.Errorf("Failed to load policy '%s'", policy)
...
// user-defined policy. This allows any explicit denies of the built-in, and user-defined policies
// to override the run-time policy. Runs normal enforcement if run-time policy is empty.
func (e *Enforcer) CreateEnforcerWithRuntimePolicy(policy string) CasbinEnforcer {
	return e.getCabinEnforcer(policy)
}
...
// by Argo Rollouts RBAC. It will find entries in the given data
...
func newAdapter(builtinPolicy, userDefinedPolicy, runtimePolicy string) *argorolloutsAdapter {
	return &argorolloutsAdapter{
...
func (a *argorolloutsAdapter) LoadPolicy(model model.Model) error {
...
func (a *argorolloutsAdapter) SavePolicy(model model.Model) error {
	return errors.New("not implemented")
}

func (a *argorolloutsAdapter) AddPolicy(sec string, ptype string, rule []string) error {
	return errors.New("not implemented")
}

func (a *argorolloutsAdapter) RemovePolicy(sec string, ptype string, rule []string) error {
	return errors.New("not implemented")
}

func (a *argorolloutsAdapter) RemoveFilteredPolicy(sec string, ptype string, fieldIndex int, fieldValues ...string) error {
	return errors.New("not implemented")
}
