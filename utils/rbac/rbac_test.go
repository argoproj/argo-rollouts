...
func TestBuiltinPolicyEnforcer(t *testing.T) {
	...
	assert.False(t, enf.Enforce("admin", "rollouts", "get", "foo/bar"))

	// now set builtin policy
	_ = enf.SetBuiltinPolicy(assets.BuiltinPolicyCSV)

	allowed := [][]interface{}{
		{"admin", "rollouts", "get", "foo/bar"},
		{"admin", "rollouts", "promote", "foo/bar"},
		{"role:readonly", "rollouts", "get", "foo/bar"},
		{"role:admin", "rollouts", "get", "foo/bar"},
		{"role:admin", "rollouts", "promote", "foo/bar"},
	}
	...
	disallowed := [][]interface{}{
		{"role:readonly", "rollouts", "create", "foo/bar"},
		{"role:readonly", "rollouts", "promote", "foo/bar"},
	}
...
`
	...
	assert.True(t, enf.Enforce("bob", "rollouts", "promote", "bar/obj"))
	assert.False(t, enf.Enforce("bob", "rollouts", "promote", "foo/obj"))
	assert.True(t, enf.Enforce("alice", "rollouts", "promote", "foo/obj"))
	assert.False(t, enf.Enforce("alice", "rollouts", "promote", "bar/obj"))
}
...
`
	...
	assert.False(t, enf.Enforce("alice", "rollouts", "promote", "bar/obj"))
	assert.False(t, enf.Enforce("alice", "rollouts", "get", "bar/obj"))
...
`
	...
	assert.False(t, enf.Enforce("mike", "rollouts/resources", "promote", "foo/obj"))
	assert.False(t, enf.Enforce(nil, "rollouts/resources", "promote", "foo/obj"))
	assert.False(t, enf.Enforce(&jwt.RegisteredClaims{}, "rollouts/resources", "promote", "foo/obj"))

	enf.EnableEnforce(false)
	assert.True(t, enf.Enforce("alice", "rollouts", "get", "foo/obj"))
	assert.True(t, enf.Enforce("alice", "rollouts/resources", "promote", "foo/obj"))
	assert.True(t, enf.Enforce("mike", "rollouts", "get", "foo/obj"))
	assert.True(t, enf.Enforce("mike", "rollouts/resources", "promote", "foo/obj"))
	assert.True(t, enf.Enforce(nil, "rollouts/resources", "promote", "foo/obj"))
	assert.True(t, enf.Enforce(&jwt.RegisteredClaims{}, "rollouts/resources", "promote", "foo/obj"))
}
...
func TestUpdatePolicy(t *testing.T) {
	...
	assert.False(t, enf.Enforce("alice", "rollouts", "get", "foo/obj"))
	assert.True(t, enf.Enforce("bob", "rollouts", "get", "foo/obj"))
...
func TestClaimsEnforcerFunc(t *testing.T) {
	...
	assert.False(t, enf.Enforce(&claims, "rollouts", "get", "foo/bar"))
	enf.SetClaimsEnforcerFunc(func(claims jwt.Claims, rvals ...interface{}) bool {
		return true
	})
	assert.True(t, enf.Enforce(&claims, "rollouts", "get", "foo/bar"))
}
...
func TestDefaultRoleWithRuntimePolicy(t *testing.T) {
	...
	assert.False(t, enf.EnforceRuntimePolicy("", runtimePolicy, "bob", "rollouts", "get", "foo/bar"))
	enf.SetDefaultRole("role:readonly")
	assert.True(t, enf.EnforceRuntimePolicy("", runtimePolicy, "bob", "rollouts", "get", "foo/bar"))
}
...
func TestClaimsEnforcerFuncWithRuntimePolicy(t *testing.T) {
	...
	assert.False(t, enf.EnforceRuntimePolicy("", runtimePolicy, claims, "rollouts", "get", "foo/bar"))
	enf.SetClaimsEnforcerFunc(func(claims jwt.Claims, rvals ...interface{}) bool {
		return true
	})
	assert.True(t, enf.EnforceRuntimePolicy("", runtimePolicy, claims, "rollouts", "get", "foo/bar"))
}
...
func TestInvalidRuntimePolicy(t *testing.T) {
	...
	require.NoError(t, enf.syncUpdate(fakeConfigMap(), noOpUpdate))
	_ = enf.SetBuiltinPolicy(assets.BuiltinPolicyCSV)
	assert.True(t, enf.EnforceRuntimePolicy("", "", "admin", "rollouts", "update", "foo/bar"))
	assert.False(t, enf.EnforceRuntimePolicy("", "", "role:readonly", "rollouts", "update", "foo/bar"))
	badPolicy := "this, is, not, a, good, policy"
	assert.True(t, enf.EnforceRuntimePolicy("", badPolicy, "admin", "rollouts", "update", "foo/bar"))
	assert.False(t, enf.EnforceRuntimePolicy("", badPolicy, "role:readonly", "rollouts", "update", "foo/bar"))
}

func TestValidatePolicy(t *testing.T) {
	goodPolicies := []string{
		"p, role:admin, rollouts, promote, *, allow",
		"",
		"#",
		`p, "role,admin", rollouts, promote, *, allow`,
		` p, role:admin, rollouts, promote, *, allow `,
...
func TestEnforceErrorMessage(t *testing.T) {
	...
	err = enf.EnforceErr("admin", "rollouts", "get", "foo/bar")
	require.Error(t, err)
	assert.Equal(t, "rpc error: code = PermissionDenied desc = permission denied: rollouts, get, foo/bar", err.Error())
...
func TestDefaultGlobMatchMode(t *testing.T) {
	...
	require.NoError(t, enf.syncUpdate(fakeConfigMap(), noOpUpdate))
	policy := `
p, alice, rollouts, get, "https://github.com/*/*.git", allow
`
	_ = enf.SetUserPolicy(policy)

	assert.True(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argoproj/argo-rollouts.git"))
	assert.False(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argoproj/argo-rollouts.git"))
}
...
`
	...
	assert.True(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argoproj/argo-rollouts.git"))
	assert.False(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argo-rollouts.git"))
}
...
func TestRegexMatchMode(t *testing.T) {
	...
	require.NoError(t, enf.syncUpdate(cm, noOpUpdate))
	policy := `
p, alice, rollouts, get, "https://github.com/argo[a-z]{4}/argo-[a-z]+.git", allow
`
	_ = enf.SetUserPolicy(policy)

	assert.True(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argoproj/argo-rollouts.git"))
	assert.False(t, enf.Enforce("alice", "rollouts", "get", "https://github.com/argoproj/1argo-rollouts.git"))
}
...
func TestLoadPolicyLine(t *testing.T) {
	t.Run("Valid permission line", func(t *testing.T) {
		policy := `p, role:Myrole, rollouts, *, myproj/*, allow`
	...
	t.Run("Invalid policy line missing policy type", func(t *testing.T) {
		policy := ", role:Myrole, rollouts, *, myproj/*, allow"
		model := newBuiltInModel()
		require.Error(t, loadPolicyLine(policy, model))
	})
}
