...
func RolloutPluginTimedOut(rolloutPlugin *v1alpha1.RolloutPlugin, newStatus *v1alpha1.RolloutPluginStatus) bool {
	...
	// Already timed out - but check if the RolloutPlugin spec has changed
	// If the observedGeneration doesn't match the current generation, the spec has changed
	// and we should allow the controller to re-evaluate
	if condition.Reason == RolloutPluginTimedOutReason {
		// If observedGeneration has changed, the timeout should be re-evaluated
		if newStatus.ObservedGeneration != rolloutPlugin.Generation {
			return false
		}
		// Otherwise, still consider it timed out
		return true
	}
...
func VerifyRolloutPluginSpec(rolloutPlugin *v1alpha1.RolloutPlugin, prevCond *v1alpha1.RolloutPluginCondition) *v1alpha1.RolloutPluginCondition {
	...
	// Validate Strategy - only Canary is supported
	if spec.Strategy.Canary == nil {
		message := "RolloutPlugin spec.strategy.canary is required"
		return newInvalidSpecRolloutPluginCondition(prevCond, RolloutPluginInvalidSpecReason, message)
	}
