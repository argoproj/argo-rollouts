...
func TestGetDryRunMetricNames(t *testing.T) {
			...
			DryRun: []v1alpha1.DryRun{
				{
					...
					MeasurementsLength: 100,
				},
			},
		}
		dryRunMetricsMap, isWildcardPresent := GetDryRunMetricsMap(spec.DryRun)
		assert.False(t, isWildcardPresent)
		dryRunObject, metricRunningInDryRun := dryRunMetricsMap["success-rate"]
		assert.True(t, metricRunningInDryRun)
		assert.Equal(t, dryRunObject.MeasurementsLength, 100)
	})
	t.Run("GetDryRunMetricNames returns the wildcard presence indicator", func(t *testing.T) {
		...
		assert.Equal(t, len(dryRunMetricsMap), 1)
	})
}
