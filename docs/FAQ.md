...
No, when the Rollout is first created, the controller does not use the provided strategy to bring up the first ReplicaSet. The controller tries to get the Rollout into a steady state as fast as possible by creating a fully scaled up ReplicaSet from the provided `.spec.template`. Once the Rollout has a stable ReplicaSet to transition from, the controller starts using the provided strategy to tranistion the previous ReplicaSet to the desired ReplicaSet.
