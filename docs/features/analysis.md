...
| ClusterAnalysisTemplate    | A `ClusterAnalysisTemplate` is like a `AnalysisTemplate`, but is not limited to what namespace it is in. It can be used by any `Rollout` throughout the cluster. |
