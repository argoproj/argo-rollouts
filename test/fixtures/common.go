...
// GetRolloutPluginEventReasons returns the event reasons for the RolloutPlugin.
func (c *Common) GetRolloutPluginEventReasons() []string {
	...
	var reasons []string
	for _, event := range c.events {
		if event.InvolvedObject.UID == rp.UID {
			reasons = append(reasons, event.Reason)
		}
	}
	return reasons
