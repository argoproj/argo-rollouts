...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	if rollout.Spec.Replicas == nil {
		...
		if err := c.writeBackToInformer(newRollout); err != nil {
			return err
		}
		return nil
	}
	...
	if err := c.writeBackToInformer(roCtx.newRollout); err != nil {
		return err
	}
	return nil
}
...
func (c *Controller) writeBackToInformer(ro *v1alpha1.Rollout) error {
	...
	if err != nil {
		return fmt.Errorf("converting rollout to unstructured: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("getting rollout from informer: %w", err)
	}
	...
	if ok && o.(*v1alpha1.Rollout).ResourceVersion != ro.ResourceVersion {
		return fmt.Errorf("informer resourceVersion %q is newer than the rollout resourceVersion %q", o.(*v1alpha1.Rollout).ResourceVersion, ro.ResourceVersion)
	}
	err = c.rolloutsInformer.GetStore().Update(&un)
	if err != nil {
		return fmt.Errorf("updating informer store: %w", err)
	}
