...
Argo Rollouts controller, using Rollout custom resource, provides additional deployment strategies such as Blue Green and Canary to Kubernetes users.  It tends to offer feature parity with the deployment resource along with additional deployments strategies.
...
- A user wants to run some last minute functional tests on the new version before it starts to serve production traffic.  With the BlueGreen strategy, the Rollout allows users to specify a preview service and an active service. The Rollout will configure the preview service to send traffic to the new version while the active service that receives production traffic. Once a user is comfortable with the new version, they can promote it to the active service. ([example](examples/example-rollout-bluegreen.yaml))
...
- A user wants to give a small percentage of the production traffic to a new version of their application for a couple of hours.  Afterward, they want to scale down the new version and look at some metrics to determine if the new version is performant to the old version. From there, they will decide if they want to rollout the new version to all of the production traffic or stick with the current version. With the canary strategy, the rollout can scale up a replica with the new version to achieve that percentage of traffic they want the new version to receive, wait for a user-inputted amount of time, set the percentage back to 0, and then wait to rollout out to the rest of the production traffic until the user unpauses the rollout. ([example](examples/example-rollout-canary-run-tmp-canary.yaml))

- A user wants to slow give the new version more production traffic. They start by giving it a small percentage of live traffic and wait a while before giving the new version more traffic.  As the percentage goes up, the amount of wait time goes down. Eventually, the new version will receive all the production traffic.  With the canary strategy, the user specifies the percentages they want the new version to receive and the amount of time to wait once that weight is achieved. ([example](examples/example-rollout-canary.yaml))
...
kubectl -n argo-rollouts -f https://raw.githubusercontent.com/argoproj/argo-rollouts/release-v0.1.0/manifests/install.yaml
```
