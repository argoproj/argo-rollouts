...
func collectAnalysisRuns(ch chan<- prometheus.Metric, ar *v1alpha1.AnalysisRun) {
	...
	dryRunMetricsMap, everythingRunningInDryRun := analysisutil.GetDryRunMetricsMap(ar.Spec.DryRun)
	for _, metric := range ar.Spec.Metrics {
		...
		isDryRunMetric := everythingRunningInDryRun || metricRunningInDryRun
		addGauge(MetricAnalysisRunMetricType, 1, metric.Name, metricType, fmt.Sprint(isDryRunMetric))
