...
func NewPrometheusAPI(metric v1alpha1.Metric) (v1.API, error) {
	...
	if strings.Contains(metric.Provider.Prometheus.Address, "aps-workspaces") {
		cfg := sigv4.SigV4Config{
			Region:  metric.Provider.Prometheus.Sigv4Config.Region,
			Profile: metric.Provider.Prometheus.Sigv4Config.Profile,
			RoleARN: metric.Provider.Prometheus.Sigv4Config.RoleARN,
		}
