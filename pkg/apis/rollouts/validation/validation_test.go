...
func TestCanaryScaleReporting(t *testing.T) {
	selector := &metav1.LabelSelector{
		MatchLabels: map[string]string{"key": "value"},
	}
	ro := &v1alpha1.Rollout{
	...
	t.Run("scaleReporting mode Stable with traffic routing", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.TrafficRouting = &v1alpha1.RolloutTrafficRouting{
			SMI: &v1alpha1.SMITrafficRouting{},
		}
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{Mode: v1alpha1.ScaleReportingModeStable}
		allErrs := ValidateRollout(ro)
		assert.Empty(t, allErrs)
	})
	t.Run("scaleReporting mode All without traffic routing", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{Mode: v1alpha1.ScaleReportingModeAll}
		allErrs := ValidateRollout(ro)
		assert.Empty(t, allErrs)
	})
	t.Run("scaleReporting empty mode", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{}
		allErrs := ValidateRollout(ro)
		assert.Empty(t, allErrs)
	})
	t.Run("scaleReporting mode Stable with basic canary", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{Mode: v1alpha1.ScaleReportingModeStable}
		allErrs := ValidateRollout(ro)
		assert.EqualError(t, allErrs[0], fmt.Sprintf("spec.strategy.scaleReporting.mode: Invalid value: \"Stable\": %s", InvalidCanaryScaleReportingNoTrafficRouting))
	})
	t.Run("scaleReporting mode Stable with dynamicStableScale", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.TrafficRouting = &v1alpha1.RolloutTrafficRouting{
			SMI: &v1alpha1.SMITrafficRouting{},
		}
		...
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{Mode: v1alpha1.ScaleReportingModeStable}
		allErrs := ValidateRollout(ro)
		assert.EqualError(t, allErrs[0], fmt.Sprintf("spec.strategy.scaleReporting.mode: Invalid value: \"Stable\": %s", InvalidCanaryScaleReportingDynamicStableScale))
	})
	t.Run("scaleReporting unknown mode", func(t *testing.T) {
		ro := ro.DeepCopy()
		ro.Spec.Strategy.Canary.TrafficRouting = &v1alpha1.RolloutTrafficRouting{
			SMI: &v1alpha1.SMITrafficRouting{},
		}
		ro.Spec.Strategy.Canary.ScaleReporting = &v1alpha1.ScaleReporting{Mode: "Bogus"}
		allErrs := ValidateRollout(ro)
		assert.EqualError(t, allErrs[0], fmt.Sprintf("spec.strategy.scaleReporting.mode: Invalid value: \"Bogus\": %s", InvalidCanaryScaleReportingMode))
	})
}
